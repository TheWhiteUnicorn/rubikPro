...
// Вспомогательная функция, сорирует карту ассоциаций, начиная с первого необработанного элемента (begin), и выталкивая элементы средних слоев в конец (на работает для нижнего слоя!)
void sortAssocMap(int* map, _edgeVector& edges, int begin) {
	...
	for (int i = begin; i < NUM_OF_MID_EDGES; i++) {
		...
		map[i] = mapNew[i]; // все значения сразу перекидываются в map
	}
...
void Assembler::doMidLayer(FormulaStack & res) {
	...
	// Карта, отображающая то, в каком порядке должны обрабатываться грани (начинать надо с тех, которые расположены на верхнем слое)
